...
resource "ncloud_lb" "lb" {
  ...
  subnet_no_list  = coalesce(var.subnet_no_list, values(data.ncloud_subnet.subnets).*.id)
...
resource "ncloud_lb_listener" "lb_listeners" {
  ...
  target_group_no  = try(var.listeners[count.index].target_group_no, data.ncloud_lb_target_group.target_groups[var.listeners[count.index].target_group_name].id)
